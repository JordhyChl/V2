// ignore_for_file: must_be_immutable, prefer_typing_uninitialized_variables, avoid_print, use_build_context_synchronously, unused_field, unused_local_variable, unnecessary_import
...
class HourMeterPage extends StatefulWidget {
  ...
  final bool darkMode;
  @override
  State<HourMeterPage> createState() => HourMeterPageState();
...
class HourMeterPageState extends State<HourMeterPage> {
  var size, height, width;
  bool _isError = false;
  String _errCode = '';
  late Future<dynamic> _getHourmeterReport;
  int page = 1;
  int perPage = 25;
  bool _isData = true;
  late List<ResultHourmeterReport> initHourmeterReport;
  ...
  void initState() {
    ...
    _getHourmeterReport = getHourmeterReport();
  ...
  refreshHourmeterReport(int pageR, int perPageR, String imeiR,
      String timeStartR, String timeEndR, int minDuration) async {
    Dialogs().loadingDialog(context);
    APIService()
        .getHourMeterReport(pageR, perPageR, imeiR, timeStartR, timeEndR)
        .then((snapshot) {
      if (snapshot is ErrorTrapModel) {
        Dialogs().hideLoaderDialog(context);
        setState(() {
          _isError = true;
          _errCode = '76853hourmeterreport';
        });
      }
      if (snapshot == 'false') {
        // initHourmeterReport.clear();
        setState(() {
          _isError = true;
          _errCode = '';
          _isData = false;
      ...
      // }
        ...
        setState(() {
          ...
          initHourmeterReport.addAll(snapshot.data.result);
          totalData = snapshot.data.totalAllData;
          totalHour = snapshot.data.totalHourMeter;
        });
        Dialogs().hideLoaderDialog(context);
        Navigator.of(context).pop();
      }
        ...
        if (isAdrress) {
          attrs.forEach((key, value) {
            if (key == 'content') {
              if (end) {
                setState(() {
                  initHourmeterReport[index].endAddress = value;
                });
              } else {
                setState(() {
                  initHourmeterReport[index].startAddress = value;
                });
              }
            }
          });
    ...
    if (result is ErrorTrapModel) {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          _errCode = '';
          if (end) {
            initHourmeterReport[index].endAddress = addressResult['message'];
          } else {
            initHourmeterReport[index].startAddress = addressResult['message'];
          }
  ...
  Future<dynamic> getHourmeterReport() async {
    final result = await APIService().getHourMeterReport(
    ...
    if (result == 'false') {
      ...
      setState(() {
        if (result.data.result.length == 0) {
          ...
          totalHour = result.data.totalHourMeter;
        }
              ...
              Stack(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        AppLocalizations.of(context)!.hourMeter,
                  ...
                  builder:
                    ...
                    if (snapshot.hasData) {
                      ...
                      if (snapshot.data == 'false') {
                        ...
                        initHourmeterReport = snapshot.data.data.result;
                        totalData = snapshot.data.data.totalAllData;
                        totalHour = snapshot.data.data.totalHourMeter;
                        return Column(
                          children: [
                            Padding(
                              padding: const EdgeInsets.only(
                              ...
                              child: Column(
                                children: [
                                  Text(
                                    AppLocalizations.of(context)!.totalEngineOn,
                                    style: reguler.copyWith(
                                      fontSize: 12,
                                      color: widget.darkMode
                                          ? whiteColorDarkMode
                            ...
                            Expanded(
                              child: ListView.builder(
                                  ...
                                  itemCount: initHourmeterReport.length,
                                  itemBuilder: (context, index) {
                                    LatLng cPosition = LatLng(
                                      initHourmeterReport[index].startLat,
                                      initHourmeterReport[index].startLon,
                                    );
                                    double ctrLat = (initHourmeterReport[index]
                                                .startLat +
                                            initHourmeterReport[index].endLat) /
                                        2;
                                    double ctrLon = (initHourmeterReport[index]
                                                .startLon +
                                            initHourmeterReport[index].endLon) /
                                        2;
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        _printDurationCard(Duration(
                                                                            seconds:
                                                                                initHourmeterReport[index].duration)),
                                                                        style: bold
                                                                            .copyWith(
                                                                          fontSize:
                                                                              8,
                                                                ...
                                                                Expanded(
                                                                  child: Row(
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        '${initHourmeterReport[index].averageSpeed.toString()} Km/h',
                                                                        style: bold
                                                                            .copyWith(
                                                                          fontSize:
                                                                              8,
                                                                ...
                                                                Expanded(
                                                                  child: Row(
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        '${initHourmeterReport[index].maxSpeed.toString()} Km/h',
                                                                        style: bold
                                                                            .copyWith(
                                                                          fontSize:
                                                                              8,
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Text(
                                                                        '${NumberFormat.currency(
                                                                          ...
                                                                          double.parse(
                                                                                initHourmeterReport[index].driveMilage.toString(),
                                                                              ) /
                                                                              1000,
                                                                        )} km',
                                                                        style: bold
