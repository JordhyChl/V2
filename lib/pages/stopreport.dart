// ignore_for_file: must_be_immutable, prefer_typing_uninitialized_variables, avoid_print, use_build_context_synchronously, prefer_is_empty, prefer_const_constructors, depend_on_referenced_packages
...
class StopReportPage extends StatefulWidget {
  ...
  final bool darkMode;
  @override
  State<StopReportPage> createState() => StopReportPageState();
...
class StopReportPageState extends State<StopReportPage> {
  var size, height, width;
  late Future<dynamic> _getStopReport;
  int page = 1;
  int perPage = 25;
  bool _isData = true;
  late List<ResultStopModel> initStopReport;
  late List<ResultStopModel> stopReport;
      ...
      for (var meta in metas) {
        ...
        if (isAdrress) {
          attrs.forEach((key, value) {
            if (key == 'content') {
              setState(() {
                initStopReport[index].address = value;
              });
            }
          });
        }
  ...
  Future<void> getAddress(String pLat, String pLng, int index) async {
    ...
    if (result is ErrorTrapModel) {
      ...
      if (mounted) {
        ...
        setState(() {
          initStopReport[index].address = addressResult['message'];
  ...
  refreshStopReport(int pageR, int perPageR, String imeiR, String timeStartR,
      String timeEndR, int minDuration) async {
    Dialogs().loadingDialog(context);
    APIService()
        .getStopReport(pageR, perPageR, imeiR, timeStartR, timeEndR,
      ...
      if (snapshot is ErrorTrapModel) {
        ...
        if (snapshot is MessageModel) {
          ...
          setState(() {
            ...
            initStopReport.addAll(snapshot.data.result);
            totalData = snapshot.data.totalAllData;
            totalHour = snapshot.data.totalStopTime;
          });
          Dialogs().hideLoaderDialog(context);
          Navigator.of(context).pop();
        }
  ...
  Future<dynamic> getStopReport() async {
    ...
    if (result == 'false') {
      ...
      setState(() {
        if (result is MessageModel) {
          ...
          totalHour = result.data.totalStopTime;
        }
                ...
                builder:
                  ...
                  if (snapshot.hasData) {
                    ...
                    if (snapshot.data == 'false') {
                      ...
                      initStopReport = snapshot.data.data.result;
                      totalData = snapshot.data.data.totalAllData;
                      totalHour = snapshot.data.data.totalStopTime;
                      return Container(
                        color: whiteColor,
                        child: Column(
                          children: [
                            Padding(
                              ...
                              child: Column(
                                children: [
                                  Text(
                                    AppLocalizations.of(context)!.totalStopTime,
                                    style: reguler.copyWith(
                                      fontSize: 12,
                                      color: widget.darkMode
                                          ? whiteColorDarkMode
                            ...
                            Expanded(
                              child: ListView.builder(
                                  ...
                                  itemCount: initStopReport.length,
                                  itemBuilder: (context, index) {
                                    LatLng cPosition = LatLng(
                                      initStopReport[index].lat,
                                      initStopReport[index].lon,
                                    );
                                    return Container(
